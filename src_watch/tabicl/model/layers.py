...
class MultiheadAttention(nn.MultiheadAttention):
    ...
    def __init__(self, embed_dim: int, num_heads: int, dropout: float = 0.0):
        ...
        self.use_kv_cache_eval = True

    def set_kv_cache(self, enabled: bool = True) -> None:
        """Enable or disable KV-cache optimization in eval mode."""
        self.use_kv_cache_eval = enabled
    ...
    def forward(
        ...
        return multi_head_attention_forward(
            ...
            attn_mask=attn_mask,
            rope=rope,
            use_kv_cache=(not self.training and self.use_kv_cache_eval),
        )
...
class MultiheadAttentionBlock(nn.TransformerEncoderLayer):
    ...
    def set_kv_cache(self, enabled: bool = True) -> None:
        self.attn.set_kv_cache(enabled)
...
class InducedSelfAttentionBlock(nn.Module):
    ...
    def set_kv_cache(self, enabled: bool = True) -> None:
        self.multihead_attn1.set_kv_cache(enabled)
        self.multihead_attn2.set_kv_cache(enabled)
