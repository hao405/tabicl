...
class TabICL(nn.Module):
    ...
    def set_kv_cache(self, enabled: bool = True) -> None:
        """Enable or disable attention KV-cache optimization in eval mode."""
        self.col_embedder.set_kv_cache(enabled)
        self.row_interactor.set_kv_cache(enabled)
        self.icl_predictor.set_kv_cache(enabled)
