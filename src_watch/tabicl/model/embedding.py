...
class ColEmbedding(nn.Module):
    ...
    def set_kv_cache(self, enabled: bool = True) -> None:
        self.tf_col.set_kv_cache(enabled)
...
#         self.reserve_cls_tokens = reserve_cls_tokens
#         self.in_linear = SkippableLinear(1, embed_dim)
#         self.in_ln = nn.LayerNorm(embed_dim) if norm_first else nn.Identity()  # Added LayerNorm here
...
#         src = self.in_ln(self.in_linear(features))  # Apply LayerNorm after linear transformation
#         src = self.tf_col(src, train_size)
#         weights = self.ln_w(self.out_w(src))  # Apply LayerNorm before weights
#         biases = self.ln_b(self.out_b(src))  # Apply LayerNorm before biases
#         embeddings = features * weights + biases
