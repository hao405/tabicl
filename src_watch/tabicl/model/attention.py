...
def multi_head_attention_forward(
    ...
    *batch_shape, tgt_len, embed_dim = query.shape
    ...
    need_full_projection = not (isinstance(attn_mask, int) and use_kv_cache)
    if need_full_projection:
    ...
    if isinstance(attn_mask, int):
        ...
        if use_kv_cache:
            # In split-attention inference, test tokens only attend to the train prefix.
            # We avoid projecting K,V for test tokens and reuse prefix K,V (KV cache).
            ...
            q_left = F.linear(query[..., :cut_pos, :], w_q, b_q)
            k_left = F.linear(key[..., :cut_pos, :], w_k, b_k)
            v_left = F.linear(value[..., :cut_pos, :], w_v, b_v)
            q_left = q_left.view(*batch_shape, cut_pos, num_heads, head_dim).transpose(-3, -2)
            k_left = k_left.view(*batch_shape, cut_pos, num_heads, head_dim).transpose(-3, -2)
            v_left = v_left.view(*batch_shape, cut_pos, num_heads, head_dim).transpose(-3, -2)
        else:
        ...
        if cut_pos < tgt_len:
            if use_kv_cache:
                q_right = F.linear(query[..., cut_pos:, :], w_q, b_q)
                q_right = q_right.view(*batch_shape, tgt_len - cut_pos, num_heads, head_dim).transpose(-3, -2)
            else:
